...
Always adhere to any rules or regurements set out in any claude ma Tiles when responding.
...
Run 'git status' and check the current Git branch, and abort this entire process if there are any uncommitted,
unstaged, or untracked files in the working directory. 
Tell the user to commit or stash changes before proceeding, and DO NOT GO ANY FURTHER.
...
Before making any content, switch to a new Git branch using the 'branch_name' derived from the ARGUMENTS• 

If the branch name is already taken, then append a version number to it:
e.g. 'claude/feature/card-component-01'


## Step 4 - Rpsearch the codebase
Read these files and folders before writing the spec:
- CLAUDE.md - roadmap, conventions, schema
- app and components folders - routes and structures
...
Create a markdown spec document that Plan mode can use directly and save it in the specs folder using the 'feature_slug'. 
Use the exact structure as defined in the spec template file here: @specs/template.md.
...
## Step 7 - Save the spec Save to: •claude/specs/
<steil_number>-<feature_slug>.md
